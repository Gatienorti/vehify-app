...
| `POST /report/{id}/refresh` `{transactionId, platform}` | Regenerate with current data (stale-report banner), paid via the `report_refresh` consumable — send the store transaction id and `ios`/`android`; it's verified with RevenueCat and consumed exactly once (replay → **422**, unverifiable → **402**). Clears the content and re-queues → the same generating payload; poll like after a purchase. |
